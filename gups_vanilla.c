...
int main(int narg, char **arg)
{
  ...
  // printf("Attempting to allocate table with size: %u bytes\n", nlocal);
  ...
  {
    if (me == 0)
      printf("Table allocation failed 1\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  if (!data)
  {
    if (me == 0)
      printf("Table allocation failed 2\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  if (!send)
  {
    if (me == 0)
      printf("Table allocation failed 3\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
